...
export function generateCity(collision) {
  ...
  function buildResidential(col, row) {
    ...
    if (style === 'houses') {
      ...
      const faceA = facingOutward(col, row);
  ...
  for (const { col, row } of gasSpots) {
    ...
    const bz = blockCenter(row);
    const face = facingOutward(col, row);
    const gasRy = face.ry + Math.PI;
    const f = frameAt(bx, bz, gasRy);
    f.col = col; f.row = row;
    const meta = gasStation(S, W, rng, f);
    const rot90 = Math.abs(Math.sin(gasRy)) > 0.5;
    const hw = rot90 ? meta.hd : meta.hw;
    const hd = rot90 ? meta.hw : meta.hd;
    const storeWorldX = bx + (meta.storeCz || 0) * Math.sin(gasRy);
    const storeWorldZ = bz + (meta.storeCz || 0) * Math.cos(gasRy);
    ...
    minimapBlocks.push({ x: bx, z: bz, w: B, d: B, zone: 'residential' });
  }
